...
#            print("salt:",salt)
        ...
        #   --- 21. return sig
        sig = (salt, z2_m, c1, salt, msg, tr, a_ntt, b,u)
        return sig
    ...
    def verify_msg_agg(self, b, a, u_ori, z2, z3, n_node_real, n_node_fake, flag,salt,tr,msg):
        """Verification procedure of Plover (core: verifies msg)."""
        ...
        u = self._msg_hash(salt, tr, msg)
        if u!=u_ori:
            print("Msg has been tamperd!")
            return False
        if self._check_bounds_agg(u, a, b, z2, z3, n_node_real) == False:
            return False
...
if (__name__ == "__main__"):
    d_shares = 16
    iut_agg = PloverSign(bitsec=128, q=PLOVERSIGN_Q, logdivide=37, nut=21, rep=8, ut=6,
                         up=42, n=PLOVERSIGN_N, d=d_shares)
    node_num_real = 20
    node_num_fake = 10
    msk_rsu, vk_rsu = iut_agg.keygen()
    tr_rsu = bytes(range(iut_agg.tr_sz))
    salt_rsu = iut_agg.random_bytes(iut_agg.salt_sz)
    sig_agg = []
    start_time = time.time()  # 获取当前时间
    print(f"Name:Plover_agg  Security parameter: 128 bit  d-shares: {d_shares}.")
    print(f"There are {node_num_real} sig-nodes.")
    print("Every node starts to generate signature...")
    with multiprocessing.Pool(processes=node_num_real) as pool:
        # 使用 map 收集每个进程的返回结果
        results = pool.map(process_instance, range(node_num_real))
    for sig in results:
        sig_agg.insert(0, sig)
    print("===Start to aggregate signatures===")
    h_agg = []
    a_agg = []
    b_agg = []
    z2_agg = []
    ...
    # start_time = time.time()  # 获取当前时间
    msg_rsu = bytes([ord('h'), 0, 0, 0, 0, 0, 0, 0])  # Start with 'h' prefix
    ...
    for i in range(0, len(h_agg), 3):
        ...
        tr = h_agg[i + 2]
        msg_rsu += salt + msg + tr
    #msg_rsu = iut_agg.msg_hash_agg(h_agg)
    as_agg = a_agg[0]
    bs_agg = b_agg[0]
    z2s_agg = z2_agg[0]
    z3s_agg = z3_agg[0]
...
#    print("z3:", z3s_agg)
    #    print("bs_agg",bs_agg)
    #    print("as_agg",as_agg)
    u_rsu = iut_agg._msg_hash(salt_rsu,tr_rsu,msg_rsu)
    msg_rsu_fake = bytes(range(3))
    sig_rsu = iut_agg.sign_msg(msk_rsu, tr_rsu, msg_rsu)
    a_agg.insert(0, sig_rsu[6])
    b_agg.insert(0, sig_rsu[7])
    z2_agg.insert(0, sig_rsu[1])
    z3_agg.insert(0, sig_rsu[2])
    ...
    print("Aggregate signatures is finished...")
    print("=== Verify aggregate signature===")
    rsp = iut_agg.verify_msg_agg(bs_agg, as_agg, u_rsu, z2, z3, node_num_real, node_num_fake, True,salt_rsu,tr_rsu,msg_rsu)
    print(f"The aggregate signature verification result is: {rsp}")
    # assert(rsp is True)
    ...
    print(
        f"The time between generating the aggregate signature and verifying the signature result is: {execution_time}s")
#    print(f"fake sign:{z2}")
