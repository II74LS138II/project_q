...
class NIST_PloverSign(PloverSign):
    ...
    def encode_pk(self, vk):
        ...
        (seed, t) = vk
        b = seed
        b += self._encode_bits(t, self.q_bits - self.nut)
        return b
    ...
    def encode_sk(self, msk):
        ...
        #   encode share 0
        b += self._encode_bits(s0, self.q_bits)
    ...
    def decode_pk(self, b):
        ...
        seed = b[0:self.as_sz]
        l = len(seed)
        t,pl = self._decode_bits(b[l:], self.q_bits - self.nut, self.n);
        l += pl
        vk = (seed, t)
    ...
    def decode_sk(self, b):
        ...
        for j in range(1, self.d):
            ...
            #   XOF( 'K' || index j || (0 pad)  || key_j )
            xof_in  = bytes([ord('K'), j, 0, 0, 0, 0, 0, 0]) + key
            ms[j]    = self._xof_sample_q(xof_in)

        #   decode share zero
        ms[0],sl =   self._decode_bits(b[l:], self.q_bits, self.n)
        l       +=  sl
    ...
    def encode_sig(self, sig):
        ...
        # first encode c1
        c1_ = [vc1 if vc1 >= 0 else vc1 + (1 << (self.enc_c1_bits + 1)) for vc1 in c1] # 2 complement
        s += self._encode_bits(c1_, self.enc_c1_bits + 1)

        s += self._compress(z, self.sig_sz - len(s))
    ...
    def decode_sig(self, s):
        ...
        c1, pi = self._decode_bits(s[i:], self.enc_c1_bits + 1, self.n, True)
        i += pi

        z = self._decompress(s[i:], self.sig_sz - i, self.n)
...
plover_128_1  = NIST_PloverSign(  bitsec=128, q=PLOVERSIGN_Q, logdivide=37, nut=21, rep=8, ut=27, 
				up=36, n=PLOVERSIGN_N, d=1, enc_c1_bits=3, sig_rate=37, sig_sz=14000)

plover_128_2  = NIST_PloverSign(  bitsec=128, q=PLOVERSIGN_Q, logdivide=37, nut=21, rep=4, ut=27, 
				up=36, n=PLOVERSIGN_N, d=2, enc_c1_bits=3, sig_rate=37, sig_sz=14000)

plover_128_4  = NIST_PloverSign(  bitsec=128, q=PLOVERSIGN_Q, logdivide=37, nut=21, rep=2, ut=27, 
				up=36, n=PLOVERSIGN_N, d=4, enc_c1_bits=3, sig_rate=37, sig_sz=14000)

plover_128_8  = NIST_PloverSign(  bitsec=128, q=PLOVERSIGN_Q, logdivide=37, nut=21, rep=4, ut=26, 
				up=35, n=PLOVERSIGN_N, d=8, enc_c1_bits=3, sig_rate=37, sig_sz=14000)

plover_128_16  = NIST_PloverSign(  bitsec=128, q=PLOVERSIGN_Q, logdivide=37, nut=21, rep=2, ut=26, 
				up=35, n=PLOVERSIGN_N, d=16, enc_c1_bits=3, sig_rate=37, sig_sz=14000)

plover_128_32  = NIST_PloverSign(  bitsec=128, q=PLOVERSIGN_Q, logdivide=37, nut=21, rep=4, ut=25, 
				up=34, n=PLOVERSIGN_N, d=32, enc_c1_bits=3, sig_rate=37, sig_sz=14000)
...
assert(msk2[:2] == msk[:2]) # eq of seed and t

assert(p_test._decode(msk2[2]) == p_test._decode(msk[2])) # eq s
