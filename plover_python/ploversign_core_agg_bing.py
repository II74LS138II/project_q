...
class PloverSign:
    ...
    def verify_msg(self, vk, tr, msg, sig):
        ...
        (salt, z2, z3, salt, msg, tr, a_ntt, b, u_ori) = sig
        (seed, b) = vk
    ...
    def verify_msg_agg(self, b, a, u, z2, z3,n_node_real,n_node_fake,flag):
        """Verification procedure of Plover (core: verifies msg)."""
        ...
        if flag:
            ...
            if self._check_bounds_agg(u, a, b, z2, z3,n_node_fake) == False:
                return False
...
def process_instance_sign(i):
   ...
   # print(f'name = {iut.name}')
    ...
    return sig
def process_instance_ver(i,sig,iut):
    _ = iut.verify_masg
...
if (__name__ == "__main__"):
    ...
    print(f"There are {node_num_real}s sig-nodes.")
    print("Every node starts to generate signature...")
    with multiprocessing.Pool(processes=node_num_real) as pool:
        # 使用 map 收集每个进程的返回结果
        results = pool.map(process_instance_sign, range(node_num_real))
    for sig in results:
        sig_agg.insert(0, sig)
    for sig in sig_agg:
        r = iut_agg.verify_msg(sig[5],sig[5])
...
#    print("as_agg",as_agg)
    ...
    rsp = iut_agg.verify_msg_agg(bs_agg, as_agg, u_agg, z2, z3, node_num_real,node_num_fake,True)
    ...
    print(f"The time between generating the aggregate signature and verifying the signature result is: {execution_time}s")
    sign_agg=(bs_agg, as_agg, u_agg, z2, z3)
